...
\textcolor{red}{
One major limitation to this approach of collecting application level provenance is that a user is required to have the source code of the specific application in which provenance data is required. Also, provenance collection is limited to function's exit and entry points. Provenance collection deals with data pruning by allowing a user to specify what provenance data to collect which is similar to our policy based apporach for efficient provenance data storage. However, one major limitation exists in which provenance is only  collected from function calls and the source code of the application will have to be available in order to collect provenance since provenance is collected during source code compilation.}
