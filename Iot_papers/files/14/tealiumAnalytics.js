...
var tealiumAnalytics = {
		...
		init : function() {
			...
			this.setTealiumPageTagsValues(jQuery, this.config.defaults.tagOptions.pageTagsOnly);
...
//			<div class="stats-tag-marker-page" data-tealium_utag_data='{"sheet_name": "Homepage - landing.jsp - via data attrib", "sheet_type": "home - legacy- via data attrib"}' />
		...
		setTealiumEventTagsValues: function (data4eventTags, j$, tagOptions) {
			...
			_.forEach(data4eventTags, function(data4eventTag, index) {
				try {
	    ...
	    //  setTealiumUtagData(data, jQuery, {setBaseFlags: false, setPageTags: true, setUserTags:false})
	    setTealiumUtagData: function (data, j$, tagOptions) {
	        ...
	        if (utag_data && typeof utag_data === "object" && data && typeof data === "object") {
	            retVal = true;
	            if (tagOptions.setBaseFlags && typeof tagOptions.setBaseFlags === "boolean") {
		...
		//  getTealiumUtagData(jQuery, {setBaseFlags: false, setPageTags: true, setUserTags:false})
		getTealiumUtagData: function (j$, tagOptions) {
			j$ = j$ || jQuery;
			var tagOptionsDefault = {
				setBaseFlags: true,		// === getBaseFlags
				setPageTags: true,
				setUserTags: true
			};
			tagOptions = j$.extend(tagOptionsDefault, tagOptions);
